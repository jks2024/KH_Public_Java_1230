...
//        System.out.println(a.equalsIgnoreCase("HELLO")); // 개소문자 구분하지 않음
//        System.out.println(a == c);

        // indexOf() : 문자열에서 특정 문자/문자열가 시작되는 인덱스를 리턴(반환) 합니다. (인덱스는 0부터 시작)
        String d = "Hello J Jav Java";
        System.out.println(d.indexOf("J"));

        // contains() : 문자열에서 특정 문자열 포함여부를 리턴 (대소문자를 구분 함)
        ...
        // replace(), repalceAll() : 문자열에서 특정 문자열을 다른 문자열로 대체하고자 할 때 사용
        System.out.println(str1.replaceAll("Pineapple", "Orange"));

        // substring(시작인덱스) : 문자열에서 특정 문자를 추출 할 때 사용 (시작 인덱스 부터 끝까지)
