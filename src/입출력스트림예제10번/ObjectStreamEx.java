...
// 객체 입출력 보조 스트림 : 자바는 메모리에 생성된 객체를 파일 또는 네트워크로 전송 할 수 있습니다.
// 객체는 문자가 아니기 때 문에 바이트 기반 스트림으로 출력 해야 합니다.
// 객체를 출력하기 위해서는 일렬로 연속적인 바이트로 변경해야 하는데, 이것을 직렬화(Serialization)라고 합니다.
// 반대로 파일이나 네트워크로 부터 전송된 객체를 읽을 수도 있는데, 이때 연속적인 바이트를 객체로 복원하는 역직렬화라고 합니다.
